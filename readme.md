...
### Автоматическое создание ярлыков (shorts.sh)

Для упрощения создания ярлыков используйте скрипт `shorts.sh`, который автоматически создаст файлы `run` и `go` в домашней директории с правильными путями.
...
После выполнения скрипта в домашней директории появятся:
- `~/run` - запуск деплоя проекта
- `~/go` - переход в директорию проекта с активацией виртуального окружения

Теперь из любой директории можно выполнить:
```bash
~/run   # Запустить деплой
~/go    # Перейти в проект с активацией venv
```

Переход в домашнию директорию
>~

Запуск деплоя из домашней
>./run
...
Для автоматической синхронизации dev базы данных с prod используйте команду:
...
**Что делает команда:**
1. Создает бэкап dev базы (сохраняется в `backups/dev_backup_YYYYMMDD_HHMMSS.sql`)
2. Создает дамп prod базы (сохраняется в `backups/prod_dump_YYYYMMDD_HHMMSS.sql`)
3. Очищает dev базу и восстанавливает данные из prod

**Важно:**
- Команда работает только в DEV окружении (проверяет `settings.DEV`)
- Автоматически создает бэкапы обеих баз для безопасности
- Не требует подтверждения - запускается сразу
- Использует настройки БД из `settings_local.py` (переменные `DB_*` и `DB_*_DEV`)
